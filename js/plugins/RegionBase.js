 ...
 * @param terrainTagList
 * @text 지형 태그 리스트 
 * @desc 지형 태그데이터의 리스트입니다.
 * @default []
 * @type struct<Record>[]
 *
 * @help RegionBase.js
 *
 * 지역 및 지형태그의 데이터베이스를 제공합니다.
 * 사양은 RPG Maker MV Trinity를 기본으로 하고있습니다.
 * 지역 및 지형태그를 트리거로 하여 다음과 같은 기능을 제공합니다.
 * ・이벤트, 플레이어를 대상으로 한 통행판정(4방향 포함)
 * ・사다리, 나무, 카운터, 데미지를 입는 바닥
 * ・공통이벤트의 호출(트리거 3종류)
 * ・지역에 들어와 있는 동안만 On되는 스위치
 * ・들어와 있는 동안만 유용한 특성
 * ・메모란
 *
 * 아래의 스크립트에서 데이터베이스를 스크립트와 외부 플러그인에서 참조할 수 있습니다.
 * 미설정 또는 첨자가 [0=인 경우 내용은 undefined가 되므로 설정 시 주의해 주십시오.
 *
 * $dataSystem.regions[ID];
 * $dataSystem.terrainTags[ID];
 *
 ...
 * @param id
 * @text ID
 * @desc 대상이 되는 지역과 지형 태그 ID 입니다.
 * @default 1
 * @type number
 *
 * @param name
 * @text 이름
 * @desc 관리상의 이름입니다. 딱히 의미는 없습니다.
 * @default
 *
 * @param collisionForPlayer
 * @text 플레이어 충돌 판정
 ...
 * @param through
 * @text 통과
 * @desc 통과 설정입니다. 활성화 되어있으으면 통행불가 타일을 통과할 수 있습니다. 충돌판정쪽이 우선시 됩니다.
 * @default false
 * @type boolean
 *
 * @param tileAttribute
 ...
 * @param switchId
 * @text 스위치ID
 * @desc 진입시 ON되는 스위치입니다. 이탈시 OFF가 됩니다.
 * @type switch
 *
 * @param traitsId
 * @text 특성
 * @desc 플레이어가 침범하는 경우 적용되는 특성입니다. 편의상 직업의 선택이라 하겠습니다.
 * @type class
 *
 * @param note
 * @text 메모
 * @desc 메모란입니다. 일반 데이터베이스와 마찬가지로 여기서 메타데이터를 자동생성합니다. 스크립트 사용을 상정하고 있습니다.
 * @type multiline_string
 *
 ...
 * @param trigger
 * @text 트리거
 * @desc 공통이벤트를 기동시키는 트리거 입니다.
 * @default 0
 * @type select
 * @option 지역에 진입했을 때 1번만 실행
 * @value 0
 * @option 지역의 안에 있을 때 걸을 댸 마다 실행
 * @value 1
 * @option 지역을 벗어났을때 1번만 실행
 * @value 2
 */
